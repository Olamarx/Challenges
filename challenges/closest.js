...
function  closest(strng) {
    // your code
    const splt = strng.split(' ')
    let map = splt.map(w => parseInt(w))
    // console.log(splt)
    const arrAdd = []
    for (let i = 0; i< splt.length; i++) {
        const mapChar = Array.from(map[i].toString()).map(Number)
        const add = mapChar.reduce((pre, cur) => pre + cur)
        arrAdd.push(add)
    }
    // let sort = arrAdd.sort((a, b) => {return a - b})
    // console.log(arrAdd)
    let min = Math.min(...arrAdd)
